...
{
    if (tiles_data->property_tiles == NULL) {
        return;
    }
    ...
    for (Weather weather = WEATHER_FIRST; weather < PROPERTY_WEATHER_COUNT; weather++) {
        for (Property_Type type = PROPERTY_TYPE_FIRST; type < PROPERTY_TYPE_COUNT; type++) {
            if (weather == WEATHER_FIRST) {
                free_property_type_data(tiles_data->property_tiles->active_src[type]);
            }

            free_property_type_data(tiles_data->property_tiles->src[weather][type]);
        }
    }
